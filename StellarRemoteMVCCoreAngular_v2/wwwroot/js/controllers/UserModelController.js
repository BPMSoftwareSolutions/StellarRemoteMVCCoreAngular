var mainApp = angular.module('DynamicBusinessModelWeb.UserModelController', [], function ($compileProvider) {
...
mainApp.controller('UserModelCtrl', function ($scope, $http, $window) {
        ...
        $http.get('/UserBusinessModel/UserBusinessModelVM').then(function (response) {
        ...
        $scope.connectToModel = function (id) {
            $scope.loading = true;
            $http.post('/UserBusinessModel/SelectModelToConnect', { id: id }).then(function (response) {
                if (response.data == true) {
                    $window.location.href = $window.location.protocol + "//" + $window.location.host + "/UserBusinessModel/OpenDefaultDashboard";
                }
            });
        }

        $scope.replicateModel = function (id) {
            $scope.loading = true;
            $window.location.href = $window.location.protocol + "//" + $window.location.host + "/UserBusinessModel/Replicate/" + id;
        }

        $scope.refreshModel = function (id) {
            $scope.loading = true;
            $window.location.href = $window.location.protocol + "//" + $window.location.host + "/UserBusinessModel/Load/" + id;
        }
        ...
        $scope.editObject = function (id) {
            $scope.loading = true;
            $http.post('/UserBusinessModel/SelectObjectToEdit').then(function (response) {
                $window.location.href = $window.location.protocol + "//" + $window.location.host + "/UserBusinessModel/Edit";
            });
        }

        $scope.createObject = function () {
            $scope.loading = true;
            $http.post('/UserBusinessModel/SelectObjectToCreate').then(function (response) {
                $window.location.href = $window.location.protocol + "//" + $window.location.host + "/UserBusinessModel/Create";
            });
        }
        ...
        $scope.cancelEdit = function () {
            $scope.loading = true;
            $window.location.href = $window.location.protocol + "//" + $window.location.host + "/UserBusinessModel/Display";
        };
